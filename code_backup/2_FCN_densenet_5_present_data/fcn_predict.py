...
X_bsc1 = np.concatenate((X_bsc1_temp_test, X_app_number_test), axis=1)
...
X_usage_time_new = sparse.load_npz(DATADIR_pro + 'present_data/test_sparse_matrix_times.npz').tocsr()
X_usage_duration_new = sparse.load_npz(DATADIR_pro + 'present_data/test_sparse_matrix_doration.npz').tocsr()
...
results = model.predict([X_constant, X_usage_time_new,X_usage_duration_new, X_tfidf,X_usage_time, X_usage_duration, X_act, X_bhv_log, X_bsc1, X_bsc2, X_app,   X_cv_max])
...
np.save('FCN_prob_0.6465_1.npy', proba_array_temp)
