...
def fcnmodel():
    ...
    #input_time_new, encode_time_new = dense_add(dim_time_new, [256,   128, 128,    64], 64)
    #input_duration_new, encode_duration_new = dense_add(dim_duration_new, [256,  128, 128,   64], 64)
    #input_tfidf, encode_tfidf = dense_add(dim_tfidf, [128, 128, 64,  64], 64)
    #dim_constant = 886
    #input_constant, encode_constant = dense_add(dim_constant, [256, 128, 128,  64, 64], 32)
    dim_cv_max = 5000
    input_cv_max, encode_cv_max = dense_add(dim_cv_max, [256,  128,  128, 64,  64, 64], 64)


    # --- merge ---
    merge = concatenate([encode_app, encode_time, encode_duration, encode_act, encode_bhv, encode_bsc1,  encode_bsc2, encode_app2,  encode_cv_max])
    ...
    model = Model(inputs=[input_app, input_time, input_duration, input_act, input_bhv, input_bsc1, input_bsc2,  input_app1,    input_cv_max],
...
if __name__ == '__main__':
    ...
    #print(X_app[0, :])
    # test data
    X_app_6_test = sparse.load_npz(DATADIR_pro + '/test_sparse_matrix.npz')
    ...
    for i in range(0, 5):
        ...
        early_stopping = EarlyStopping(monitor='val_out_clf_acc', min_delta=0, patience=1, verbose=1, mode='max')
        reduce_lr = LearningRateScheduler(scheduler)
        model = fcnmodel()
        model.fit(x=[X_app_6,X_usage_time, X_usage_duration, X_act, X_bhv_log, X_bsc1, X_bsc2, X_app,   X_cv_max], y=[Y, Y_1hot],
        ...
        results = model.predict([X_app_6_test, X_usage_time_test, X_usage_duration_test, X_act_test, X_bhv_log_test, X_bsc1_test, X_bsc2_test, X_app_test,   X_cv_max_test])
