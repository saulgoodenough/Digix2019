...
for data_name in datalist:
        ...
        # 计算top_n个app一个月内的使用天数

        # 计算app一个月内的使用天数
        train_1days = csr_matrix((np.ones(temp_1_train.shape[0]), (temp_1_train.trainrow, temp_1_train.appId_category)),shape=(df_train.shape[0], appIds))
        test_1days = csr_matrix((np.ones(temp_1_test.shape[0]), (temp_1_test.testrow, temp_1_test.appId_category)),shape=(df_test.shape[0], appIds))
        # 构造特征，一个月内app的使用天数
        if i == 0:
                train_30days = train_1days
                test_30days = test_1days
        else:
                train_30days += train_1days
                test_30days += test_1days
        i+=1
        usage_train_duration = str('usage') + data_name + str('_train_duration.npz')
        usage_test_duration = str('usage') + data_name + str('_test_duration.npz')
        usage_train_times = str('usage') + data_name + str('_train_times.npz')
        usage_test_times = str('usage') + data_name + str('_test_times.npz')
        print(usage_train_duration)
        print(usage_test_duration)
        print(usage_train_times)
        print(usage_test_times)
        sparse.save_npz(os.path.join(out_datadir, usage_train_duration), temp_1_train_duration)  # 保存
        sparse.save_npz(os.path.join(out_datadir, usage_test_duration), temp_1_test_duration)  # 保存
        sparse.save_npz(os.path.join(out_datadir, usage_train_times), temp_1_train_times)  # 保存
        sparse.save_npz(os.path.join(out_datadir, usage_test_times), temp_1_test_times)  # 保存
# 一个月内的使用天数
sparse.save_npz(os.path.join(out_datadir, 'train_30days.npz'), train_30days)  # 保存
sparse.save_npz(os.path.join(out_datadir, 'test_30days.npz'), test_30days)  # 保存
