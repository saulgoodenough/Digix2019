...
def stack_multiple(stack_num, stack_dim = 6):
    input_tensor_list = []
    for i in range(stack_num):
        input_tensor_list.append(Input(shape=(stack_dim,)))
...
def fcnmodel():
    ...
    dim_categ =  737
    input_categ, encode_categ = dense_add(dim_categ, [256, 128,  64], 64)
    dim_active = 5000
    input_active, encode_active = dense_add(dim_active, [256, 128,  64], 64)
    dim_time = 5000
    ...
    stack_num = 3
    stack_list = stack_multiple(stack_num, stack_dim = 6)




    # --- merge ---
    merge = concatenate([encode_cont, encode_categ, encode_active, encode_time, encode_duration]+stack_list)
...
if __name__ == '__main__':
    ...
    model.fit(x=[train_x_cont, train_x_categ, X_act_cv, train_sparse_matrix_doration_npz, train_sparse_matrix_times_npz] + stack_data_list, y=[Y, Y_1hot],
