...
def duration_sum_times(data, effctive_apps_files):
    ...
    def temp(data, data_name):
        temp_1 = data.loc[(data['date'] == data_name)]
        temp_1.reset_index(drop=True, inplace=True)
        temp_1['appId_category'] = temp_1['id'].map(dictionary)
        ...
        temp_1_train_duration = csr_matrix(
            (temp_1_train['duration'].values, (temp_1_train.trainrow, temp_1_train.appId_category)),
            shape=(data_train.shape[0], app_numbers))
        temp_1_test_duration = csr_matrix(
            (temp_1_test['duration'].values, (temp_1_test.testrow, temp_1_test.appId_category)),
            shape=(data_test.shape[0], app_numbers))
        temp_1_train_times = csr_matrix(
            (temp_1_train['times'].values, (temp_1_train.trainrow, temp_1_train.appId_category)),
            shape=(data_train.shape[0], app_numbers))
        temp_1_test_times = csr_matrix((temp_1_test['times'].values, (temp_1_test.testrow, temp_1_test.appId_category)),
                                       shape=(data_test.shape[0], app_numbers))
        user_app_usage_train_duration[data_name] = temp_1_train_duration
        user_app_usage_test_duration[data_name] = temp_1_test_duration
        user_app_usage_train_times[data_name] = temp_1_train_times
        user_app_usage_test_times[data_name] = temp_1_test_times
    ...
    with multiprocessing.Manager() as manager:
        ...
        for i, data_name in enumerate(datalist):
            print(data_name)
            thread_name = "usage_thead_%d" % i
            multipro.append(multiprocessing.Process(target=temp, name=thread_name, args=(data, data_name,)))
        for process in multipro:
            process.start()
        for process in multipro:
            process.join()
        ...
        for i, date in enumerate(datalist):
            if i == 0:
                train_duration = user_app_usage_train_duration[data_name]
                test_duration = user_app_usage_test_duration[data_name]

                train_times = user_app_usage_train_times[data_name]
                test_times = user_app_usage_test_times[data_name]
            else:
                train_duration += user_app_usage_train_duration[data_name]
                test_duration += user_app_usage_test_duration[data_name]
    ...
    sparse.save_npz('train_duration30day.npz', train_duration)  # 保存
    sparse.save_npz( 'test_duration30day.npz', test_duration)  # 保存
    sparse.save_npz('train_times30day.npz', train_times)  # 保存
    sparse.save_npz( 'test_times30day.npz', test_times)  # 保存
